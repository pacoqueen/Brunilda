...
class Selection(graphics.Sprite):
    def __init__(self, start_time = None, end_time = None):
        ...
        # El valor por defecto de la selección no puede ser None porque no 
        # se puede comparar con datetimes.
        self.start_time, self.end_time = dt.datetime.now(), dt.datetime.now()
...
class DayLine(graphics.Scene):
    ...
    def __init__(self, start_time = None, scope_hours = 24):
        graphics.Scene.__init__(self)
        # day_start = conf.get("day_start_minutes")
        day_start = 6 * 60  # El día empieza a las 6:00, valor en minutos.
    ...
    def plot(self, date, tareas, select_start, select_end = None):
        for bar in self.tarea_bars:
            self.plot_area.sprites.remove(bar)
        self.tarea_bars = []
        for tarea in tareas:
            tarea_bar = graphics.Rectangle(0, 0, fill = "#aaa", stroke="#aaa") # dimensions will depend on screen situation
            tarea_bar.tarea = tarea
            if tarea.empleado in self.empleados:
                tarea_bar.empleado = self.empleados.index(tarea.empleado)
            else:
                tarea_bar.empleado = len(self.empleados)
                self.empleados.append(tarea.empleado)
            self.plot_area.add_child(tarea_bar)
            self.tarea_bars.append(tarea_bar)
    ...
    def on_click(self, scene, event, target):
        ...
        end_time = None
        if self.tarea_bars:
            times = [bar.tarea.ini for bar in self.tarea_bars if bar.tarea.ini - start_time > dt.timedelta(minutes=5)]
            times.extend([bar.tarea.ini + bar.tarea.duracion for bar in self.tarea_bars if bar.tarea.ini + bar.tarea.duracion - start_time  > dt.timedelta(minutes=5)])
            if times:
                end_time = min(times)
        self.new_selection()
        self.emit("on-time-chosen", start_time, end_time)
        ...
        if self.current_x:
            ...
            for bar in self.tarea_bars:
                if bar.x < self.current_x < bar.x + bar.width:
                    ...
                    break
            if active_bar:
                self.set_tooltip_text("%s - %s" % (active_bar.tarea.area.nombre, active_bar.tarea.empleado.nombre))
            else:
    ...
    def on_enter_frame(self, scene, context):
        ...
        for bar in self.tarea_bars:
            bar.y = vertical * bar.empleado + 5
            bar.height = vertical
            bar_start_time = bar.tarea.ini - self.view_time
            minutes = bar_start_time.seconds / 60 + bar_start_time.days * self.scope_hours * 60
            bar.x = round(minutes / minute_pixel) + 0.5
            bar.width = round((bar.tarea.duracion).seconds / 60 / minute_pixel)
            if not snap_points or bar.x - snap_points[-1][0] > 1:
                snap_points.append((bar.x, bar.tarea.ini))
            if not snap_points or bar.x + bar.width - snap_points[-1][0] > 1:
                snap_points.append((bar.x + bar.width, bar.tarea.ini + bar.tarea.duracion))
